...
public class EnemyManager : MonoBehaviour, ISaveable, IObserverDeath
{
        ...
        for (int i = 0; i < colliders.Length; i++) {
            if (colliders[i].tag == "Player") {
                ...
                if ((viewableAngle > minimumDetectionAngle && viewableAngle < maximumDetectionAngle) || 
                    ...
                    RaycastHit hit;
                    Ray ray = GetRayTowardsTarget(colliders[i].bounds.max.y);
    ...
    private void AttackBehaviour()
    {
        ...
        {
            RaycastHit hit;
            Ray ray = GetRayTowardsTarget(currentTarget.GetComponent<Collider>().bounds.max.y);
    ...
    private Ray GetRayTowardsTarget(float targetHeight) =>
        new Ray(eyePosition.position, ((currentTarget.transform.position +
